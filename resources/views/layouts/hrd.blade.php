...
<head>
    ...
    <title>{{ $title ?? 'HRD Executive Panel - E-Recruitment PT Sariling Aneka Energi' }}</title>
...
<body class="flex h-screen overflow-hidden bg-slate-100 text-slate-800 antialiased">
    ...
    <!-- Main HRD Content Area -->
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        <!-- Topbar HRD -->
        <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
            <div class="flex items-center space-x-4">
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">{{ $header ?? 'Executive HRD Panel' }}</h1>
            </div>
            <div class="flex items-center space-x-4">
